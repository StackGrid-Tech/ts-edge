...
declare const graphStore: <T extends GraphStoreState = GraphStoreState>(initializer: GraphStoreInitializer<T>) => GraphStore<T>;
 ...
 */
type GraphNodeMiddleware<T extends GraphNode> = (node: GraphNodeWithOutOutput<T>, next: (route?: GraphNodeWithOutOutput<T>) => void) => any;
/**
 ...
 */
type GraphNode<Name extends string = string, Input = any, Output = any, Metadata extends GraphNodeMetadata = GraphNodeMetadata> = {
 ...
 */
type InputOf<T extends GraphNode, Name extends T['name']> = Extract<T, {
    name: Name;
}>['input'];
/**
 ...
 */
type OutputOf<T extends GraphNode, Name extends T['name'] = T['name']> = Extract<T, {
    name: Name;
}>['output'];
/**
 ...
 */
type GraphEvent<T extends GraphNode = GraphNode, StartNodeName extends T['name'] = T['name'], EndNodeName extends T['name'] = T['name']> = GraphStartEvent<InputOf<T, StartNodeName>> | GraphEndEvent<T, OutputOf<T, EndNodeName>> | GraphNodeStartEvent<T> | GraphNodeEndEvent<T> | GraphNodeStreamEvent<T>;
/**
 ...
 */
type GraphNodeRouter<AllNode extends GraphNode, FromNodeName extends AllNode['name'], ConnectableNodeName extends AllNode['name']> = Router<Extract<AllNode, {
    name: FromNodeName;
}>['output'], ConnectableNodeName>;
/**
 ...
 */
interface GraphDefaultRunnable<T extends GraphNode = never, StartNode extends T['name'] = never, EndNode extends T['name'] = never> {
    /**
     ...
     */
    run(input?: Partial<InputOf<T, StartNode>>, options?: Partial<GraphRunOptions>): Promise<GraphResult<T, OutputOf<T, EndNode>>>;
 ...
 * @template EndNode - The name of the ending node
 */
interface GraphRunnable<T extends GraphNode = never, StartNode extends T['name'] = never, EndNode extends T['name'] = never> extends GraphDefaultRunnable<T, StartNode, EndNode> {
    /**
     * Executes the graph workflow.
     * @param input - Input data for the starting node
     * @param options - Optional configuration for execution
...
interface StateGraphRunnable<T extends GraphNode = never, StartNode extends T['name'] = never, EndNode extends T['name'] = never> extends GraphDefaultRunnable<T, StartNode, EndNode> {
    run(input?: Partial<T['input']>, options?: Partial<GraphRunOptions & {
        noResetState?: boolean;
    }>): Promise<GraphResult<T, T['output']>>;
    use(middleware: (node: GraphNodeWithOutOutput<T>, next: (route?: GraphNodeWithOutOutput<T>) => void) => any): StateGraphRunnable<T, StartNode, EndNode>;
}
/**
...
interface GraphRegistry<T extends GraphNode = never, Connected extends string = never> {
    ...
    addMergeNode<Name extends string, Branch extends T['name'][], Output = any>(mergeNode: {
        ...
        }, context: GraphNodeExecuteContext) => Output;
        metadata?: GraphNodeMetadata;
     ...
     * @returns Updated graph registry
     */
    edge<FromName extends Exclude<T['name'], Connected>, ToName extends Exclude<ConnectableNode<T, Extract<T, {
        name: FromName;
    }>>, FromName>>(from: FromName, to: ToName | ToName[]): GraphRegistry<T, Connected | FromName>;
    /**
     ...
     * @returns Updated graph registry
     */
    dynamicEdge<FromName extends Exclude<T['name'], Connected>, PossibleNode extends ConnectableNode<T, Extract<T, {
        name: FromName;
    }>>[]>(from: FromName, routerOrConfig: GraphNodeRouter<T, FromName, ConnectableNode<T, Extract<T, {
        name: FromName;
    }>>> | {
     ...
     */
    compile<StartName extends string = T['name'], EndName extends string = T['name']>(startNode: StartName, endNode?: EndName): GraphRunnable<T, StartName, EndName>;
}
/**
 ...
 * @template Connected - Names of nodes that already have outgoing connections
 */
interface StateGraphRegistry<T extends GraphStoreState = GraphStoreState, NodeName extends string = never, Connected extends string = never> {
    /**
    ...
    addMergeNode<Name extends string, Branch extends NodeName[]>(mergeNode: {
        ...
        }, context: GraphNodeExecuteContext) => any;
        metadata?: GraphNodeMetadata;
    }): StateGraphRegistry<T, NodeName | Name, Connected>;
    /**
     ...
     * ```
     */
    edge<FromName extends Exclude<NodeName, Connected>, ToName extends Exclude<NodeName, FromName>>(from: FromName, to: ToName | ToName[]): StateGraphRegistry<T, NodeName, Connected | FromName>;
    /**
     ...
     * ```
     */
    dynamicEdge<FromName extends Exclude<NodeName, Connected>, PossibleNode extends NodeName[]>(from: FromName, routerOrConfig: Router<T, NodeName> | {
        possibleTargets: [...PossibleNode];
     ...
     * ```
     */
    compile<StartName extends NodeName = NodeName, EndName extends NodeName = NodeName>(startNode: StartName, endNode?: EndName): StateGraphRunnable<GraphNode<NodeName, T, T>, StartName, EndName>;
}

declare const graphNode: <Name extends string = string, Input = any, Output = any, Metadata extends GraphNodeMetadata = GraphNodeMetadata>(node: {
    name: Name;
    execute: (input: Input, context: GraphNodeExecuteContext) => Output;
    metadata?: Metadata;
}) => {
...
declare function graphNodeRouter<PossibleNode extends string[]>(possibleTargets: [...PossibleNode], router: GraphNodeRouter<any, any, PossibleNode[number]>): any;
